...
let telaFimDeJogo = document.getElementById("fim-de-jogo");
...
function atualizar() {
    ...
    contexto.fillText("Canos:" + pontuacao, 5, 45);
...
function ajustarDificuldade() {
    if (valorTotalApostado > 100) {
        porcentagemVelocidade = valorTotalApostado * 0.1;
        velocidadeX = -2 - porcentagemVelocidade * 0.1;
    }
...
function mostrarTelaDerrota() {
    if ( pontuacao < 10 || pontuacao == 0) {
        pontuacaoFinal.innerHTML = "Você passou por " + pontuacao + " Canos";
        mensagem.innerHTML = "Você não conseguiu multiplicar seu dinheiro!!";
        quantiaTotal.innerHTML = "0";
    } else{
        console.log(pontuacao)
        pontuacaoFinal.innerHTML = "Você passou por " + pontuacao + " Canos";
        while (pontuacao > 10) {
            valorTotalApostado = valorTotalApostado * 2;
            pontuacao = pontuacao - 10;
            console.log("MULTIPLICOU " + pontuacao);
        }

        mensagem.innerHTML = "Parabéns!! O valor final foi: " + valorTotalApostado +"R$";
    }
    telaFimDeJogo.style.visibility = 'visible';
}

function esconderTelaDeDerrota(){
    telaFimDeJogo.style.visibility = 'hidden';
}
